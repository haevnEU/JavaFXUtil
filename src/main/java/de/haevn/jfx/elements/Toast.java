...
public class Toast {
    ...
    private static Toast toastingInstance = new Toast();
    private final Stage stage = new Stage();
    private final Deque<ToastContainer> stack = new ArrayDeque<>();

    private Toast(final Stage owner) {
        stage.initOwner(owner);
        owner.xProperty().addListener((observable, oldValue, newValue) -> toastingInstance.render());
        owner.yProperty().addListener((observable, oldValue, newValue) -> toastingInstance.render());
        owner.widthProperty().addListener((observable, oldValue, newValue) -> toastingInstance.render());
        owner.heightProperty().addListener((observable, oldValue, newValue) -> toastingInstance.render());
    }
    ...
    public static void initialize(final Stage owner) {
        toastingInstance = new Toast(owner);
    }
    ...
    private static void internalToastIt(final String message, final Type type, ToastDuration duration) {
        if (null == toastingInstance) throw new IllegalStateException("Toast not initialized");
        toastingInstance.toastIt(message, type, duration);
    ...
    private static final class ToastContainer {
        ...
        ToastContainer(final String message, final Type type, ToastDuration duration) {
            toastStage.initOwner(toastingInstance.stage.getOwner());
        ...
        public void render(final int id) {
            final var owner = toastingInstance.stage.getOwner();
